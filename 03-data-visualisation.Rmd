...
What are LSOAs? They are small geographical areas developed census information in order to calculate a range of statistics on neighbourhoods. You will see that by using the `range()` function (`range(burglary_df$IMDrank)`), that not all LSOAs are included, as this dataset only contains cases from Greater Manchester. 
...
In your group google doc, type out the following: the number of entries for each dataframe. Type out the number following n = . In addition, state what you think the unit of analysis is in each dataframe.
