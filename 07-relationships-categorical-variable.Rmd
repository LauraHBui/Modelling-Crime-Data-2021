...
cohen.d(compliance_score ~ prepost| Subject(subject_id), data = df)
...
The **chi-square statistic** is a test of statistical significance for two categorical variables. It tells us how much the observed distribution differs from the one expected under the null hypothesis. It is based on the cross-tabulation or **contingency table**. These appear as a table that shows the frequency distributions of more than one variable simultaneously. 

Frequency distributions are a useful way of exploring categorical variables that do not have too many categories. By extension, **cross-tabulations** are a useful way of exploring relationships between two categorical variables that do not have too many levels or categories. This semester we are only going to explore **two-way cross-tabulations**, that is contingency tables where we plot the frequency distribution of two variables at the same time. 
