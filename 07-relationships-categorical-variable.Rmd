...
The variable, `tcviolent`, represents 'respondent level of worry about being a victim of personal crime (high score = high worry)'. This is a numeric variable. Now we observe how scores from `tcviolent` might differ between sexes in the variable `sex`. We can visualise a five-number summary with boxplots (see Lesson 3, section 3.3.3.3):
<br>
...
We observe that males seem to report lower scores of worry. Let us now look at the mean, standard deviation, and variance: 
<br>
...
The mean for females is higher than that of males, and we have quite high dispersion too, as indicated by the variance. Note that these descriptive statistics are only about our sample; we actually want to make inferences about our population of interest -- all males and females in Britain. So, we use inferential statistics. 

Hence, our hypothesis is that, in the population, there is a difference in the mean level of worry about victimisation for males compared to that for females. 
...
Our non-directional hypothesis has to do with our research question of whether females are more worried about being a victim of crime than males. We test our hypothesis using the **t-test**, which relies on the t-Student distribution. 
...
The reason is that the SD varies from sample to sample, so this variation in smaller samples makes for more uncertainty. In addition, we are likely to not know the true population values, our parameters. For such a case, William Gosset, [Statistician and Head Brewer for Guinness,](https://mathshistory.st-andrews.ac.uk/Biographies/Gosset/) (see also recommended reading) suggested that we needed to use a different probability distribution, **Student's *t* distribution**. Gosset had developed this distribution (under his pen name 'Student') so that he could make meaningful predictions using small samples of barley instead of large ones -- a cost-effective method.
...
The t-test makes a number of assumptions:

1. Two means or proportions will be compared: our independent variable is binary, signifying two groups, and the dependent variable is either a numeric variable OR a binary variable. 

2. Normal distribution is assumed for both populations (from which the two groups come from): this assumption, however, can be relaxed if the sample size for both groups are large. Because the t-test makes assumptions about the shape of the distribution, it is considered a **parametric test**. Tests that do not make this assumption about shape are called non-parametric. 

3. The two groups must be independent of each other: This assumption of independence would be violated if, for example, one group comprised women and the other group, their partners. If this were the case, the resulting values for both groups would be deceptively related. Similarly, if we compare an individual before and after a treatment, the obtained values would be related. (For these situations, however, we would need the dependent t-test, which we will not cover in this class.)

4. The sampling method was *independent random sampling*. The BCS uses a complex survey design which would require the use of special procedures for hypothesis testing. However, those special procedures are well beyond the scope of this course and would typically be covered in more advanced courses. Therefore, for convenience we will proceed as if this assumption is met.
...
According to the output, there are `r BCS0708 %>% group_by(sex) %>% summarise(n = n()) %>% filter(sex == "male") %>% pull(n)` males and `r BCS0708 %>% group_by(sex) %>% summarise(n = n()) %>% filter(sex == "female") %>% pull(n)` females. 
...
```{r, echo = FALSE}
# Checking out DV by variable ‘sex’
knitr::kable(BCS0708 %>% group_by(sex) %>% skim(tcviolent))
...
From the output, it seems that men have a lower average fear of crime scored than women do, specifically a difference of a score of `r round(abs(BCS0708 %>% group_by(sex) %>% summarise(foc_mean = mean(tcviolent, na.rm = T)) %>% filter(sex == "female") %>% pull(foc_mean) - BCS0708 %>% group_by(sex) %>% summarise(foc_mean = mean(tcviolent, na.rm = T)) %>% filter(sex == "male") %>% pull(foc_mean)),3)`. The difference between the medians is even larger. `r round(abs(BCS0708 %>% group_by(sex) %>% summarise(foc_mean = median(tcviolent, na.rm = T)) %>% filter(sex == "female") %>% pull(foc_mean) - BCS0708 %>% group_by(sex) %>% summarise(foc_mean = median(tcviolent, na.rm = T)) %>% filter(sex == "male") %>% pull(foc_mean)),3)`. So this is about our sample - but the true question is - Is this a statistically significant difference?
