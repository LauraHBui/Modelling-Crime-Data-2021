...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
nys_1 <- read_dta(here("Datasets", "nys_1_ID.dta"))
nys_2 <- read_dta(here("Datasets", "nys_2_ID.dta"))                 
...
Before we conduct the test to tell us this this, we conduct the **test for equality of variance**. This is an F-test that evaluates the null hypothesis that the variances of the two groups are equal. When we want to compare variances, we conduct this test as the variances may affect how we specify our t-test.
...
$H_0$: There is no significant difference in the number of times a youth reported stealing something worth more than $50 between Wave 1 and in Wave 2. 

<div style="margin-bottom:15px;">
</div> 

$H_A$: There is a significant difference in the number of times a youth reported stealing something worth more than $50 between Wave 1 and in Wave 2.
...
The results indicate that we fail to reject the null hypothesis as the p-value is above α = 0.05. We conclude that there is not a significant difference in thefts of items totaling more than $50 between the paired Waves 1 and 2 responses.
...
(1) compares these expected frequencies with the ones we actually observe in each of the cells, (2) then averages the differences across the cells, and (3) produces a standardised value, *χ*$^2$
