...
>>>>>>> 676cd110ae17daacbbecb7926494a9479c4d7587
...
From the above, we created a dataset of all 3.6 million probationers in the US. This is our **population**. When we look at the mean, median, and standard deviation of this population, these numbers are the *true* estimates of IQ scores in the population of American probationers. 
...
The SE is `r sd(new_1000_sample$IQ)/sqrt(1000)`. What does this mean? Well remember that the standard error is the standard deviation of the sampling distribution of our sample statistic (here the mean). And remember what percentage of observations, within a normal distribution, fall within some standard deviations away from the mean?
...
So, with our standard error above, we can say that almost all of the samples (95%) will produce a statistic (in this case a mean) which are within +/- 2* `r sd(new_1000_sample$IQ)/sqrt(1000)`, so within +/- `r 2*sd(new_1000_sample$IQ)/sqrt(1000)` or about 1 IQ point away from the true mean IQ for the whole population of 3.6 million probationers!
...
So here, we can conclude from our sample that the mean IQ for all probationers will be somewhere between `r mean(sample1$IQ, na.rm = TRUE) - 1.96*sd(sample1$IQ)/sqrt(100)` and `r mean(sample1$IQ, na.rm = TRUE) + 1.96*sd(sample1$IQ)/sqrt(100)/sqrt(100)`. 
...
Looks like our samples fared better than we would hope, in this example `r table(new.sample.ci100$capture.mean)[2]`% contained our true population mean, and  `r table(new.sample.ci100$capture.mean)[1]`% did not, but it's still close to what we expected. What if we were to do this with 1000 samples of 100? What do you think the yes vs nos would look like then? What about 10,000 samples? Discuss in your groups. 
