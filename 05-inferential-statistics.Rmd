...
The SE (of the mean) is `r sd(new_1000_sample$IQ)/sqrt(1000)`. How to interpret and communicate this estimate? A way of talking about this has to do with the **68-95-99 rule**, or known as an empirical rule.
...
With our above SE, and because we are dealing with the normal distribution, we can say that 95% of the sample will produce a mean which is within above or below 2 * `r sd(new_1000_sample$IQ)/sqrt(1000)`; in other words, within +/- `r 2*sd(new_1000_sample$IQ)/sqrt(1000)`, or within approximately one IQ point above and below the mean IQ for the whole population of 3.6 million probationers -- this is close to the true estimate.
...
We can conclude from our sample that the mean IQ for all probationers will be somewhere between `r mean(sample1$IQ, na.rm = TRUE) - 1.96*sd(sample1$IQ)/sqrt(100)` and `r mean(sample1$IQ, na.rm = TRUE) + 1.96*sd(sample1$IQ)/sqrt(100)/sqrt(100)`. 
...
- **Sample 2**: we conclude that the mean IQ for all probationers will be somewhere between `r mean(sample2$IQ, na.rm = TRUE) - 1.96*sd(sample2$IQ)/sqrt(100)` and `r mean(sample2$IQ, na.rm = TRUE) + 1.96*sd(sample2$IQ)/sqrt(100)`. 
- **Sample 3**: we conclude that the mean IQ for all probationers will be somewhere between `r mean(sample3$IQ, na.rm = TRUE) - 1.96*sd(sample3$IQ)/sqrt(100)` and `r mean(sample3$IQ, na.rm = TRUE) + 1.96*sd(sample3$IQ)/sqrt(100)`. 
...
According to our tiny table, about 95% of the CIs from our samples contained the true population estimate. In this example `r table(new.sample.ci100$capture.mean)[2]`% contained our true population mean, and  `r table(new.sample.ci100$capture.mean)[1]`% did not, and it is close to what we expected. What if we were to do this with 1,000 samples of 100? What do you think the tiny table of 'yes' and 'no' would look like then? What about 10,000 samples? Discuss in your groups. 
