...
m.reoffs.gg <- ggplot(f.df,aes(x=f.count,y=Freq*100)) + 
                  ...
                  ggtitle('Simulated Distribution of Re-offending')
...
So we get this test statistic of `r z_stat`, but how can we interpret this? Well you can use this z-test statistic to find *the associated p-value*. How can we do that? 
...
You can see that the value is 0.2302. This is a more precise approximation than our lookup table (where we had to round `r z_stat` to -0.7), and so we are getting a more precise p-value. 
