...
k <- 100 # Number of young people
# If f replications have 100 young people each, what percent of the replications will have m re-offenders in samples of 100 young people?
...
# Create the ggplot
m.reoffs.gg <- ggplot(f.df,aes(x=f.count,y=Freq*100)) + 
                  geom_bar(stat='identity',fill='black', col = "white") + 
                  ylab('Frequency (%)') + xlab('Number of reoffenders (out of 100 sample)') + 
                  ggtitle('Figure 6.3: Simulated Distribution of Re-offending')
...
The output shows the shaded area at `r pnormGC(124, region="below", mean=iq_m, sd=iq_sd,graph=FALSE) `, meaning that the prisoner has a higher IQ than over `r round(pnormGC(124, region="below", mean=iq_m, sd=iq_sd,graph=TRUE)*100 ,1)`94% of the prison population. 
...
The officer conducts an IQ assessment of all 233 prisoners at their prison and finds average IQ is `r mean(prison_1$IQ)` (SD = `r sd(prison_1$IQ)`). 
...
We obtained this test statistic of `r z_stat`, but how can we interpret this? We can use z-test statistic to find *the associated p-value*.  
...
The value is `r z_stat_1`. This is a more precise approximation than our lookup table (where we had to round `r z_stat`), and so we are getting a more precise p-value. 
