...
One way of determining whether a case is an outlier is if it is above or below 1.5 * by the **interquartile range** (IQR), or for extreme outliers, above or below 3 * IQR. 
...
+ value = means distribution is positively skewed or skewed right
- value = means distribution is negatively skewed or skewed left 
...
Now, for example, we are interested in knowing whether the modal category describes the overall work histories prior to arrest of federal inmates well, or if there is a lot of variation in responses from inmates. To do so, we use the variable `V1748`:
...
Now, the **variance** is about spread, specifically the sum of the squared deviations from the mean, then divided by the total number of cases. We use the function `var()` from the `stats` `r library(stats)` package that comes already preinstalled. 
