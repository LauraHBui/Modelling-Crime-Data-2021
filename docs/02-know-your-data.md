...
Another example: to create the online tutorial for this class, we have a project called `Modelling-Crime-Data-2021`. Figure 2.7 shows that this title appears in the top right hand corner and its contents appear in under the *Files* tab in the *Files, Plots...* pane. 
...
How do you ask `R` what your variable is ? Let us find out how using our `ncvs` data frame. 
...
*Scenario 1*: we want a variable like the `injured` variable, but for the values, we instead see 'uninjured' and 'injured' and not '0' and '1';
...
Although we know what the numbers represent, we might forget or someone else who is viewing the variable may not know. In this case, it would be helpful to change the numbers '0' and '1' to what they represent.
...
View the `ncvs` data frame. Notice that a new column, `injured_new` appeared at the end with the exact same contents as the variable `injured`. Now we recode to change those '0' and '1' values.

A function to recode is `as_factor()`, also from the `haven` package. This function takes the labels assigned to the values of the variable, and recodes the original values into these labels. 
...
This function tells us that `injured`, a binary variable, is stored as numbers, where the 0 value means the victim was uninjured and the 1 value means they were injured. Also, the `weaponpresent` variable is a binary variable stored as numbers. Here, more victims report that the offender did not use a weapon during the offence (n= 15,814) as opposed to using one (n= 6652). In addition, there are a number of missing values for this question (n= 1503).
...
Now to Scenario 3: we want to turn the variable `relationship` into a binary variable called `notstranger` whereby the offender was a stranger (0) or was known to the victim (1). 
...
Think of `case_when()` like an ‘if’ logical statement. It allows us to make changes to a variable that are conditional on some requirement. For example, we specify that values greater than '0' (values 1 to 3) mean 'not a stranger' and values equal to '0' mean 'stranger':
...
Returning to a familiar variable, the output in the console shows that 'uninjured' is labelled '0' and 'injured' is labelled '1'. Maybe, though, you do not like the labels that are attached to the variable values. Perhaps they do not make sense or they do not help you to understand better what this variable measures. If so, we can remove and change the labels. 
...
![**Figure 2.10**](https://oliviergimenez.github.io/intro_tidyverse/assets/img/logo_pipe.png)
...
To get more rows, you can use 'from:to' notation. To get the first two rows, for example, you say 'from 1 to 2', that is '1:2':
...
You can combine these by taking `slice()` and combining it with `select()`. For example:
