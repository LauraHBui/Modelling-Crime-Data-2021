...
Let us say we want to learn about these variables. The first step to choosing the appropriate analysis for our variables, `Q_10A` and `Q_12` is establishing: *what is their levels of measurement*?  

To review levels of measurement, refer to Lesson 2 (section 2.4.1). Have a think and record your answers in the google doc.
<br>
...
Now we have established the level of measurement for our variables, what type of object does `R` think these variables are? In other words, what are these variables classed as? Recall that we can use the `class()` function to answer this: 
<br>
...
The factor variable has two levels: '(1) Agency has acquired in any form (including testing)' and '(2) Agency has not acquired'. 
...
```r
# Double check that the package modeest is loaded
...
Indeed, there are 2,013 observations where the agency had not had any body worn cameras, which is more than the 1,915 observations where the agency had body worn cameras in any format.

We could also run the `mlv()` function and save the output to an object rather than print the answers into the console. Here, we save to the object called `mode_adopted`: 
<br>
...
Above we used the functions `min()` and `max()`, and inside we put the name of the specific variable we want the minimum and maximum of and the name of the data frame to which it belongs (hence, `bwcs$Q_12`). We also added a code `na.rm=TRUE`after the comma. 

What the heck is `na.rm`? When calculating measures of central tendency, you need to tell `R` that you have missing (NA) observations. Using `na.rm = TRUE` will exclude observations that have already been defined as missing. If we do not specify this code, it will instead return `NA` for agencies that have acquired BWCs because `Q_12` has missing data. 
...
Now let us find the average number of BWCs used by agencies that have responded ‘Yes, acquired’ in `Q10_A`. We will use two ways to determine the mean: the `dplyr` way and the usual `base R` way.
...
Now if we want to calculate these descriptive statistics at once?  This is where the `dplyr` way is most handy. We specify that we want to create four columns that will appear in the output as a table: 
<br>
...
What exactly constitutes an *outlier*? Generally, an outlier is any observation that shows an extreme value on one of our variables. There are different approaches to define what is an outlier, but, in this class, we use the **interquartile range** (IQR) to do so. 
...
A popular method for determining outliers is known as **Tukey fences**. According to this method, outliers are seen as falling within a lower or an upper fence. They are calculated from the following:
<br>
...
In addition, extreme outliers can be calculated from the following:
<br>
...
We obtain the IQR of the `Q_12` variable using the `IQR` function, and save this into an object called `bwc_deployed_iqr`:
<br>
...
These new objects are now in our environment (that Environment pane in the top right). We can look at their characteristics on the number of BWCs with the `summary()` function: 
<br>
...
![**Figure 4.4** Police Body Worn Cameras](Images/bwcs.jpg){width=70%}
...
What is meant by dispersion is the spread of values from the one value chosen to represent them all. That one value is usually the mean, or in the case of categorical variables, the mode. This is important because the single number to summarise our data might often mask variation. Last semester, we read *The Tiger That Isn't* and you might remember the section about the 'white rainbow'. We know the vibrancy of each of the colours of the rainbow, but if we combined them, they would form a bland white band in the sky - the average of all the colours is white. If we just represented the rainbow by the average, we would miss quite a lot!

<br> 

![**Figure 4.5** Double rainbow?!](Images/rainbow.jpg)
...
1.	Put the data set 'da04572.0001 to' into an object called `inmatesurvey04` by using the `<-` assignment operator. 
<br>
...
Then, we check if we have cases defined as missing using `is.na` to get a summary of missing cases:
<br>
...
This tells us that there are 2,530 complete cases and 1,156 missing values. 
...
What about the variation ratio? To get this, we create two data frames and count the number of people who reported any employment -- answers of (1) Full-time  (2) Part-time and (3) Occasional) in one data frame, and only the numbers who reported the modal category, which is (1) Full-time, in the other data frame. 
...
Now, get the number of observations who reported being employed prior to arrest. Store this value in the object `n_employed`: 
<br>
...
Now let us create a new variable so we do not write over the old one. We do this so we can code '997' and '998' values as missing - you can see these stand for 'Refused' and 'Don't know' respectively.
<br>
