...
The output shows the shaded area at 0.9453207, meaning that the prisoner has a higher IQ than over 94.594\% of the prison population.
...
The officer conducts an IQ assessment of all 233 prisoners at their prison and finds average IQ is 101.3733906 (SD = 15.1621288).
...
We obtained this test statistic of 1.3826487, but how can we interpret this? We can use z-test statistic to find \emph{the associated p-value}.
...
The value is 0.9166137. This is a more precise approximation than our lookup table (where we had to round 1.3826487), and so we are getting a more precise p-value.
...
\includegraphics{07-relationships-categorical-variable_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\includegraphics{07-relationships-categorical-variable_files/figure-latex/unnamed-chunk-9-1.pdf}
...
## F = 1.9022, num df = 99, denom df = 99, p-value = 0.001556
## alternative hypothesis: true ratio of variances is not equal to 1
## 95 percent confidence interval:
##  1.279891 2.827139
## sample estimates:
...
## t = 17.408, df = 99, p-value < 2.2e-16
## alternative hypothesis: true difference in means is not equal to 0
## 95 percent confidence interval:
##  28.39663 35.70290
## sample estimates:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{BCS0708}\SpecialCharTok{$}\NormalTok{bcsvictimR }\OtherTok{\textless{}{-}} \FunctionTok{as\_factor}\NormalTok{(BCS0708}\SpecialCharTok{$}\NormalTok{bcsvictim)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{BCS0708}\SpecialCharTok{$}\NormalTok{urban }\OtherTok{\textless{}{-}} \FunctionTok{as\_factor}\NormalTok{(BCS0708}\SpecialCharTok{$}\NormalTok{rural2)}
\FunctionTok{print}\NormalTok{(}\FunctionTok{levels}\NormalTok{(BCS0708}\SpecialCharTok{$}\NormalTok{urban))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mytable}\FloatTok{.3}\OtherTok{\textless{}{-}}\FunctionTok{table}\NormalTok{(BCS0708}\SpecialCharTok{$}\NormalTok{urban, BCS0708}\SpecialCharTok{$}\NormalTok{bcsvictimR)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mytable}\FloatTok{.4}\OtherTok{\textless{}{-}}\FunctionTok{table}\NormalTok{(BCS0708}\SpecialCharTok{$}\NormalTok{urban, BCS0708}\SpecialCharTok{$}\NormalTok{bcsvictim)}
\FunctionTok{print}\NormalTok{(mytable}\FloatTok{.4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
