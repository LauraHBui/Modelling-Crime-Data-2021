...
We use the \texttt{do\ ()} and \texttt{group\_by\ ()} functions to demonstrate:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# We use the do () function to make 1,000 automatic resamples from the data frame ‘prob\_off’}
\CommentTok{\# Each resample comprises 100 cases }
\CommentTok{\# We put our 1,000 resamples into an object called ‘sample100’}
...
\CommentTok{\# Additional variables are now found in sample100, which helps us to calculate the mean IQ of each sample, for which there are now 1000 of them}

\CommentTok{\# Now select the sample100 data frame}
...
\FunctionTok{mean}\NormalTok{(sample\_means100}\SpecialCharTok{$}\NormalTok{meanIQ)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# We can visualise this sampling distribution to compare to the previous population distribution}
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ sample\_means100) }\SpecialCharTok{+} 
...
\includegraphics{05-inferential-statistics_files/figure-latex/unnamed-chunk-14-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sd}\NormalTok{(sample\_means300}\SpecialCharTok{$}\NormalTok{meanIQ)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{05-inferential-statistics_files/figure-latex/unnamed-chunk-18-1.pdf}
...
\NormalTok{new.sample.ci100 }\OtherTok{\textless{}{-}}\NormalTok{ sample\_means100 }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{slice}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{100}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#Take the first 100 means }
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{lower =}\NormalTok{ meanIQ}\FloatTok{{-}1.96}\SpecialCharTok{*}\FunctionTok{sd}\NormalTok{(meanIQ), }\AttributeTok{upper =}\NormalTok{ meanIQ}\FloatTok{+1.96}\SpecialCharTok{*}\FunctionTok{sd}\NormalTok{(meanIQ), }\CommentTok{\# Compute lower and upper boundaries to create a new variable called ‘capture.mean’}
         \AttributeTok{capture.mean =} \FunctionTok{if\_else}\NormalTok{(}\AttributeTok{condition =}\NormalTok{ lower }\SpecialCharTok{\textgreater{}}\NormalTok{ true.mean }\SpecialCharTok{|}\NormalTok{ upper }\SpecialCharTok{\textless{}}\NormalTok{ true.mean, }\AttributeTok{true =} \StringTok{"no"}\NormalTok{, }\AttributeTok{false =} \StringTok{"yes"}\NormalTok{)) }\CommentTok{\# Specify code below to be: If lower \textgreater{} true mean or upper \textless{} true mean then capture.mean will \#be "yes" }
\CommentTok{\# If not, capture.mean will be "no"}


\CommentTok{\# Produce table to show new variable and how many Cis captured population mean}
