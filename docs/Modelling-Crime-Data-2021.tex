...
The output shows the shaded area at 0.9450601, meaning that the prisoner has a higher IQ than over 94.594\% of the prison population.
...
The officer conducts an IQ assessment of all 233 prisoners at their prison and finds average IQ is 100.6824034 (SD = 14.8464117).
...
We obtained this test statistic of 0.7016131, but how can we interpret this? We can use z-test statistic to find \emph{the associated p-value}.
...
The value is 0.7585397. This is a more precise approximation than our lookup table (where we had to round 0.7016131), and so we are getting a more precise p-value.
...
\hypertarget{total-number-of-activities-12-2}{%
\paragraph*{\texorpdfstring{\emph{Total number of activities}: 12}{Total number of activities: 12}}\label{total-number-of-activities-12-2}}
\addcontentsline{toc}{paragraph}{\emph{Total number of activities}: 12}
...
\item
  \texttt{plot\_model()} : Plots odds ratios and their confidence intervals (\texttt{sjPlot})
\item
...
In science, one of our main concerns is to develop \textbf{models} of the world -- models that help us to better understand the world or to predict how things will develop. You can read more about modelling in scientific research \href{https://www.visionlearning.com/en/library/Process-of-Science/49/Modeling-in-Scientific-Research/153}{here}. Statistics provides a set of tools that help researchers build and test scientific models.
...
\caption{\textbf{Figure 9.1} Simple Model}
\end{figure}
...
\caption{\textbf{Figure 9.2} A relatively complex model}
\end{figure}

In this lesson, we are going to cover regression analysis or, rather, we are will begin to talk about it.

Specifically, we start with a popular technique called \textbf{ordinary least squares (OLS)} regression. It is used to explore whether one or multiple independent variables (IV or \(X\)) can predict or explain the variation in the dependent variable (DV or \(Y\)).

When multiple IVs are included simultaneously, they are called \textbf{covariates}; when only one IV is used, the OLS regression is called \textbf{bivariate regression} or \emph{simple regression}. OLS regression has been the mainstay analysis in the social sciences.

\hypertarget{activity-1-setting-up-the-r-prep-routine}{%
\subsubsection{\texorpdfstring{Activity 1: Setting up! The \texttt{R} prep routine}{Activity 1: Setting up! The R prep routine}}\label{activity-1-setting-up-the-r-prep-routine}}
...
\item
  Load the dataset `csew1314\_teaching.csv', using the \texttt{read\_csv()} function
\item
...
Some variables of interest that we could explore are perception of antisocial behaviour (\texttt{antisocx}) and whether it is associated with other variables such as age (\texttt{age}), gender (\texttt{gender}), victimisation experience (\texttt{bcsvictim}), and confidence in the criminal justice system (\texttt{confx}).

For example, if we start with perception of antisocial behaviour (\texttt{antisocx}) and age (\texttt{age}), we would visualise our variable distributions:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Histogram for age}
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{mapping =} \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ age)) }\SpecialCharTok{+} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Histogram for \textquotesingle{}antisocx\textquotesingle{}}
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{mapping =} \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ antisocx)) }\SpecialCharTok{+} 
  \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{binwidth =}\NormalTok{ .}\DecValTok{2}\NormalTok{) }\CommentTok{\# antisocx is negatively skewed}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{activity-3-exploring-your-data-further-deleting-missing-data}{%
\subsubsection{Activity 3: Exploring your data further: deleting missing data}\label{activity-3-exploring-your-data-further-deleting-missing-data}}
...
\FunctionTok{summary}\NormalTok{(df}\SpecialCharTok{$}\NormalTok{age) }\CommentTok{\# NA = 188}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{summary}\NormalTok{(df}\SpecialCharTok{$}\NormalTok{antisocx) }\CommentTok{\# NA = 26695}
\end{Highlighting}
\end{Shaded}
...
Both variables have missing data. You could ignore the `NAs', as when we run regression, it will conduct a \emph{listwise deletion}, where observations are dropped from your analysis if they have missing data on either the IV (\(X\)) or the DV (\(Y\)). But as we want descriptive statistics of only observations we will be using, we will delete those that will be dropped anyway.

We use the \texttt{complete.cases()} function to keep cases that have valid responses in both the IV and DV:
...
\hypertarget{activity-4-what-questions-can-regression-answer}{%
\subsubsection{Activity 4: What questions can regression answer?}\label{activity-4-what-questions-can-regression-answer}}

For this example, we are interested in the bivariate relationship between age (\(X\)) and perception of antisocial behaviour (\(Y\)).

Our research question is: `Does age \emph{predict} perceived level of antisocial behaviour in one's neighbourhood?' Our null and alternative hypotheses are as follows:
...
Returning to the concept of models as to answer `what questions can regression answer?', a model can be anything that allows for inferences about a population (from a sample) to be drawn. For example, earlier in the semester, we used the mean, which could be used to draw inferences. Here, we are interested developing a model to predict perception of antisocial behaviour (\texttt{antisocx}).
...
A model helps our guesses become more accurate. Once we model the relationship between \texttt{antisocx}, this dependent variable, and an independent variable, we can use the model to help improve our guesses. Let us start by looking at a scatterplot:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Checking out how the variables covary together }
\CommentTok{\# (See Lesson 8, section 8.2.1.1 on covariation)}
...
We now draw a trend line through the scatterplot, which represents the mean of the variable \texttt{antisocx} (on the y-axis) for each age:
...
\includegraphics{09-regression_files/figure-latex/unnamed-chunk-10-1.pdf}

The red \textbf{regression line} produces guesses or predictions for the value of perceived level of antisocial behaviour based on information that we have about age.
...
\hypertarget{assumption-1-independence-of-errors}{%
\paragraph*{Assumption 1: Independence of errors}\label{assumption-1-independence-of-errors}}
\addcontentsline{toc}{paragraph}{Assumption 1: Independence of errors}

Errors of the prediction, which make up the regression line, are assumed to be independent of each other. The term \emph{heteroscedasticity} is used to describe this violation of independence of errors. When we violate this assumption, our points in the scatterplot resemble the shape of a funnel. If there is dependency between the observations, you would need to use other models that are not covered in this class.

\hypertarget{assumption-2-equal-variances-of-errors}{%
\paragraph*{Assumption 2: Equal Variances of errors}\label{assumption-2-equal-variances-of-errors}}
\addcontentsline{toc}{paragraph}{Assumption 2: Equal Variances of errors}

If the variance of our residuals (and what they are is explained later) is unequal, we will need different estimation methods, but this issue is minor. The reason it is a minor issue is that regression is considered a \emph{robust} estimation, meaning that it is not too sensitive to changes in the variance.

\hypertarget{assumption-3-normality-of-errors}{%
\paragraph*{Assumption 3: Normality of errors}\label{assumption-3-normality-of-errors}}
\addcontentsline{toc}{paragraph}{Assumption 3: Normality of errors}
...
\hypertarget{assumption-4-multicollinearity}{%
\paragraph*{Assumption 4: Multicollinearity}\label{assumption-4-multicollinearity}}
\addcontentsline{toc}{paragraph}{Assumption 4: Multicollinearity}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Inputting our variables of interest in a model and placing in object called \textquotesingle{}cal\_vif\textquotesingle{}}
...
\CommentTok{\# Running VIF analysis}
...
\hypertarget{assumption-5-linearity}{%
\paragraph*{Assumption 5: Linearity}\label{assumption-5-linearity}}
\addcontentsline{toc}{paragraph}{Assumption 5: Linearity}
...
To resolve this issue, the error values are squared before they are summed. This is called the \emph{error sum of squares}. Our regression model is actually trying to find a line of best fit that has the least (squared) error. Hence, least squared estimation.
...
To fit the regression model, we use the \texttt{lm()} function using the formula specification \texttt{(Y\ \textasciitilde{}\ X)}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Placing model into an object called \textquotesingle{}fit\_1\textquotesingle{}}
...
In the output above we saw something called \textbf{residuals}. The residuals are the differences between the observed values of \(Y\) for each observation minus the predicted or expected values of \(Y\). In other words the distances between each point in the dataset and the regression line (see Figure 9.3).

\begin{figure}
\centering
\includegraphics{Images/residual.jpg}
\caption{\textbf{Figure 9.3} Residuals}
\end{figure}
...
\caption{\textbf{Figure 9.4} R-squared}
\end{figure}

We can use information from the residuals to produce a measure of \emph{effect size}, of how good our model is in predicting variation in our dependent variable. This is \emph{\(R^2\)}.
...
The distance (squared differences) between that mean value of \(Y\) and the observed values of \(Y\) is what we call the \textbf{total variation} (total deviation in Figure 9.4). Total variation comprises two parts: explained and unexplained variation.
...
To produce \emph{\(R^2\)}, we would contrast total variation and explained variation. The formula is as follows:

\(R^2 = \dfrac{SSR}{SST} = \dfrac{\Sigma(\hat y_i - \bar y )^2}{\Sigma(y_i - \bar y )^2}\)

Whereby SSR is sum of squares regression and SST is sum of squares total. All this formula is doing is taking a ratio of the explained variation (SSR) by the total variation (SST). This gives us a measure of the \emph{percentage of variation in \(Y\) that is `explained' by \(X\)}.
...
For this example, we include additional variables to our model: sex (\texttt{sex}), previous victimisation in past 12 months (\texttt{bcsvictim}), and confidence in the police (\texttt{confx}). First, we check out these variables and tidy them as necessary:
...
It could not be any easier to fit a multiple regression model. You simply modify the formula in the \texttt{lm()} function by adding the additional variables, separated with a \texttt{+}. Let us run this to predict antisocial behaviour with all our independent variables:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Placing multiple regression into object called \textquotesingle{}fit\_2\textquotesingle{}}
...
\CommentTok{\# Running the multiple regression}
...
You may also like to present your results visually with a plot. For this, you can use the \texttt{plot\_model()} function of the \texttt{sjPlot} package, which makes it easier to produce these sorts of plots. You can find a more detailed tutorial about this function \href{https://strengejacke.wordpress.com/2017/10/23/one-function-to-rule-them-all-visualization-of-regression-models-in-rstats-w-sjplot/}{here}.
...
\includegraphics{09-regression_files/figure-latex/unnamed-chunk-18-1.pdf}
...
There are other packages that also provide similar functionality, like the \texttt{dotwhisker} package that you may want to explore. (More details \href{https://cran.r-project.org/web/packages/dotwhisker/vignettes/dotwhisker-vignette.html}{here}.)
...
For this example, we are interested in the relationship between race and receiving harsher treatment for possession of marijuana. We use the dataset \texttt{Arrests} from the \texttt{effects} package. This data includes information on police treatment of individuals arrested for possession of marijuana in Toronto, Canada.
...
Recall from OLS regression that we assessed how well our overall model predicts the DV by observing the \emph{F-statistic} and, especially, the \emph{\(R^2\)}. This is not the case for logistic regression. The reason is, in our familiar linear regression, we understand residual variation (related to that regression line) through the error sum of squares.
...
\hypertarget{activity-12-quantitative-prediction}{%
\subsubsection{Activity 12: Quantitative Prediction}\label{activity-12-quantitative-prediction}}
...
\includegraphics{09-regression_files/figure-latex/unnamed-chunk-27-1.pdf}
