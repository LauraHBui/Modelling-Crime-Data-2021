...
One way to address this is self-report victimisation surveyrs, such as the \href{https://www.crimesurvey.co.uk/en/index.html}{Crime Survey for England and Wales}. While it might be ideal to survey everyone in the UK about whether they have been a victim of robbery in the past year and how many times they have been robbed, you might imagine, this is quite impractical. Surveying the entire \textbf{population} is not very practical because of time and financial constraints.
...
A \textbf{sample} of the population is a small selection of that population of interest. You may recall from last semester and from your first year research methods courses about different approaches to sampling - that is different ways to select the people who you want to ask the questions to. In the case of such surveys, the aim is to choose the best sampling method to promote \textbf{external validity}.

If our sample has high \textbf{external validity}, then those included in it are a good representation of the population. To establish external validity depends on the way you collect your sample (e.g., random sampling versus convenience samples). This will make conclusions from a sample generalisable to the population. That is what the \emph{Crime Survey of England and Wales} does: sample from the population to get an estimate of how widespread crime and victimisation are in the whole of the population.
...
The synthetic data will consist og randomly generated numbers to represent the intelligence quotient (IQ) scores of every probationer in the US, which is a population of about 3.6 million. For this example, we assume the mean IQ scores to be 100 and the standard deviation to be 15. We create this population distribution by using the function \texttt{nrnorm()} and assigning this to a vector object called \texttt{prob\_iq}. Within the \texttt{nrnorm()} function, we specify the parameters \texttt{n\ =}, \texttt{mean\ =}, and \texttt{sd\ =}. That is the \textbf{n}umber of observations we want (3.6 million, one for each of the probationers in the US, remember this is the \textbf{population}), the \textbf{mean} IQ score we want the population to have (that is 100, specified above), and the dispersion around this mean, given by the standard deviation in the \texttt{sd\ =} parameter (specified above as 15 IQ points).
...
\tightlist
\item
  1: the mean is not \textbf{exactly} 100 and the sd not \textbf{exactly} 15. But they are very very close.
\item
  2: your answers may be slightly different to the lab notes. This is because r is \emph{randomly} generating these numbers for you, every time you ask it to. So if you re-run the code above to re-create your `` object, you will get yet another set of numbers!
\end{itemize}
...
So, if want our results to be the same, so we set a seed using the function \texttt{set.seed()}, which ensures it generates the exact same distribution for this session. Then we re-create the \texttt{prob\_iq} object:
...
Great! Now we have these random IQ scores, let's build a data frame using the function \texttt{data.frame()} in which we will create 2 columns. One for a unique identifier for each probationer. Let's call this column \texttt{probationer\_id} and another for IQ let's call this one \texttt{IQ}. Name the whole dataframe \texttt{prob\_off}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{prob\_off }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{probationer\_id =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{3600000}\NormalTok{,      }\CommentTok{\# create a column and fill with all the numbers from 1 to 3.6 million}
                       \AttributeTok{IQ =}\NormalTok{ prob\_iq )     }\CommentTok{\# create a column and fill with our random IQ scores}
\end{Highlighting}
\end{Shaded}

Now we are nearly there with our fake population data, but for one last step. The values for IQ score must be whole numbers (integers). To achieve this, we will use the \texttt{round()} function. In the \texttt{round()} function you specify two things, first the object you wish to round (the numbers in the IQ column of our prob\_off dataframe so \texttt{prob\_off\$IQ}) and the number of decimals you would like displayed (since we want whole numbers, we want 0 decimals!).
...
Now we finally have our complete fake population of 3.6 million US probationers and their IQ scores. You can have a look at this data with the \texttt{View()} function. Let's also visualise this distribution will help us identify its shape. We use \texttt{ggplot2}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Have you loaded the \textquotesingle{}ggplot2\textquotesingle{} library?}
...
So above we created a dataset of all probationers in the US, all 3.6 million of them. This is our \textbf{population}. When we look at the mean, median, and standard deviation of this population, these numbers are the \emph{true} estimates of IQ scores in the population of American probationers.
...
In the \texttt{sample()} function, we specify two parameters. \texttt{x\ =} specifies what data set to sample from, and \texttt{size\ =} specifies the size of the sample to take. So in this case, to randomly select 100 probationers from the \texttt{prob\_off} dataframe we use:
...
Now that we have our sample \texttt{sample1}, we can take some descriptive statistics of `sample1'
...
Now that we have our second, sample \texttt{sample2}, we can take some descriptive statistics once more:
...
Again, we are getting slightly different numbers, as we have sampled a separate 100 probationers from our population of 3.6 million. Depending on which sample we chose, our conclusions would be different. This variation in estimates is known as \textbf{sampling variability}, an unavoidable consequence of randomly sampling observations from the population.
...
Sampling variability makes up what is known as the \textbf{sampling distribution}. Sampling distribution of a sample statistic (for example the mean) refers to the distribution of that value if we were to take many many many many many many many samples, and each time, record the value. Then if we took all of these values, they will follow a normal distribution, the mean of which will be the true population value! This is really cool, and we will explore it in the next activity.

\hypertarget{activity-4-the-sampling-distribution}{%
\subsubsection{Activity 4: The sampling distribution}\label{activity-4-the-sampling-distribution}}
...
Remember the function to get one sample of 100 people above? It was \texttt{sample(x\ =\ prob\_off,\ size\ =\ 100)}. Now to repeat this 1,000 times we can use the \texttt{do()} function, which tells R to do something multiple times. So to take 1,000 samples of 100 probationers we want to \emph{do} \texttt{sample(x\ =\ prob\_off,\ size\ =\ 100)} 1,000 times, like so:
...
This might take a while (you are sampling 1,000 times after all!) so give R time to process. When done, you will see the sample1000 object appear in your environment. Have a look at it with \texttt{View()}.

You can see we have the prisoner ID, and we have the IQ score, but we also have these new variables, \texttt{.row} and \texttt{.index}. \texttt{.row} refers to the probationer's position in their particular sample, while \texttt{.index} refers to the sample into which they belong. So if the \texttt{.index} says 1, they were in the 1st sample, 2, the 2nd sample, and so on up to 1000. To select a particular sample for example the 2nd one, you could use the \texttt{filter()} function in order to select all cases where \texttt{.index\ ==\ 2}. But we want to keep all 1,000 samples. Instead, what we want is the \textbf{average IQ for each sample}.

You may recall how to get the mean for each value of a categorical variable from last week! Specifically, we used \texttt{group\_by()} and \texttt{summarise()}. Let's use these again, to create a new object, \texttt{sample\_means1000}, which has the mean IQ for each sample (all 1,000 of them!):
...
The resulting dataframe (\texttt{sample\_means1000}) has 2 columns, one for sample id and one for the mean score of IQ for that specific sample. It has 1,000 observations - one for each sample!
...
The histogram of our sampling distribution shows this very important concept in inferential statistics: in the case of an \textbf{unbiased estimator} what we see is that whenever we draw a sample, sometimes we \emph{over estimate} the parameter, and sometimes we \emph{under estimate} the parameter. In this case, this means that sometimes our sample mean is \emph{larger} than our population mean, and other times our sample mean is \emph{smaller} than the population mean. Usually the mean of the randomly selected sample will fall close to the populatin mean, but occasionally, it will fall far. Now - what is really exciting, is that if you randomly draw repeated samples from the same population and calculate the mean of each sample, then plot the frequency of those means, you will get the \emph{normal distribution} -- that bell-shaped curve! It doesn't even matter if your underlying data are normally distributed! Your sample statistics will be!
...
\textbf{However} in real life, you are not ever going to conduct 1,000 surveys on 100 probationers each. Instead, you will have one survey, and you will have to make sure that your one sample is a good one. Random sampling is one approach, we see here, but another thing to think about is that of the \textbf{sample size}. The next activity explores this.
...
Let's make the 3 samples new dataframe objects, and call them \texttt{sample30}, \texttt{sample100}, and \texttt{sample1000}:
...
We now have 3 dataframes, each with 1,000 samples, but one with 30 people per sample, one with 100 peope per sample, and one with 1,000 people per sample. Let's bind these together, but first, for each one, create a new column called ``sample\_size'' which tell us which one has how many people in each sample.
...
We use the function \texttt{bind\_rows()} to \emph{bind} these different data frames of different sample sizes into one complete dataframe, to answer our question:
...
So how do we know what sample size is big enough? For this, we carry out something known as a \textbf{power analysis} but we will get to that next week. For now, think ``bigger is better'' when it comes to sample size!

So great, we can say all these things in our hypothetical world of the many many many samples from the same population. But like we said earlier, we don't really get a chance to do that in the real world. So how can we trust our samples? To do this, we can quantify our sample variabilty using \textbf{standard errors}.
...
One way to clearly quantify and communicate our uncertainty is to use \textbf{confidence intervals} (CIs). These appear as an interval that tells you the margin of error -- far away is your sample statistic from the population parameter. We calculate them by, first, returning to our normal distribution, and its \textbf{68-95-99 rule}, or known as an empirical rule, which states that 68\% of cases in the distribution will fall within one standard deviation above and below the mean; 95\% within two SD; and 99\% within three SD.
...
Let's look back at our \texttt{sample1} object. Then take the mean of IQ in this sample:
...
\tightlist
\item
  \textbf{Sample 2}: we conclude that the mean IQ for all probationers will be somewhere between 97.8539159 and 103.7660841.
\item
  \textbf{Sample 3}: we conclude that the mean IQ for all probationers will be somewhere between 96.3651461 and 102.4548539.
\end{itemize}
...
\NormalTok{new\_sample\_100 }\OtherTok{\textless{}{-}} \FunctionTok{do}\NormalTok{(}\DecValTok{1000}\NormalTok{) }\SpecialCharTok{*} \FunctionTok{sample}\NormalTok{(prob\_off, }\AttributeTok{size =} \DecValTok{100}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Select the sample of 1,000 samples, each with 100 probationers and place in object, ‘new.sample.ci100’}
...
If you have a look at this new dataframe \texttt{new.sample.ci100} you can see that for each one of our 100 samples, we have the sample mean, as well as the sd and the calculated upper and lower CI. Let's revisit what we learned from recoding, and use the \texttt{if\_else()} function to create an additional variable, which tells us whether or not each one of the CIs contains the true population mean (\texttt{true.mean} above).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{new.sample.ci100 }\OtherTok{\textless{}{-}}\NormalTok{ new.sample.ci100 }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{capture.mean =} \FunctionTok{if\_else}\NormalTok{(}\AttributeTok{condition =}\NormalTok{ lower\_ci }\SpecialCharTok{\textgreater{}}\NormalTok{ true.mean }\SpecialCharTok{|}\NormalTok{ upper\_ci }\SpecialCharTok{\textless{}}\NormalTok{ true.mean, }\AttributeTok{true =} \StringTok{"no"}\NormalTok{, }\AttributeTok{false =} \StringTok{"yes"}\NormalTok{)) }\CommentTok{\# Specify code below to be: If lower \textgreater{} true mean or upper \textless{} true mean then capture.mean will \#be "yes" }
\CommentTok{\# If not, capture.mean will be "no"}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{05-inferential-statistics_files/figure-latex/unnamed-chunk-36-1.pdf}
...
\CommentTok{\# Place data frame in object called ‘PrisonerIQ’}
\NormalTok{PrisonerIQ }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{( }\AttributeTok{prisoner\_id =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{1500000}\NormalTok{, }\AttributeTok{IQ =} \FunctionTok{round}\NormalTok{(}\FunctionTok{rnorm}\NormalTok{(}\DecValTok{1500000}\NormalTok{, }\AttributeTok{mean =} \DecValTok{100}\NormalTok{, }\AttributeTok{sd =} \DecValTok{15}\NormalTok{), }\DecValTok{0}\NormalTok{)) }
...
\NormalTok{IQ\_Over\_100 }\OtherTok{\textless{}{-}}\NormalTok{ PrisonerIQ[ }\FunctionTok{which}\NormalTok{(PrisonerIQ}\SpecialCharTok{$}\NormalTok{IQ}\SpecialCharTok{\textgreater{}}\DecValTok{100}\NormalTok{) ,] }

\CommentTok{\# Divide ‘IQ\_Over\_100’ by total number of prisoner IQ scores}
\FunctionTok{nrow}\NormalTok{(IQ\_Over\_100)}\SpecialCharTok{/}\FunctionTok{nrow}\NormalTok{(PrisonerIQ)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{PrisonerIQ}\SpecialCharTok{$}\NormalTok{IQ[}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{115}

\CommentTok{\# Create a variable storing z{-}scores of IQs }
\NormalTok{PrisonerIQ}\SpecialCharTok{$}\NormalTok{z\_scoreIQ }\OtherTok{\textless{}{-}} \FunctionTok{scale}\NormalTok{(PrisonerIQ}\SpecialCharTok{$}\NormalTok{IQ) }

\CommentTok{\# Check to make sure prisoner \#1 has a z{-}score around 1 }
\NormalTok{PrisonerIQ[}\DecValTok{1}\NormalTok{,]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# The mean should be 100 but, in reality, it is not precise, so we calculate it and put in the \#object ‘m’}
\NormalTok{m}\OtherTok{\textless{}{-}}\FunctionTok{mean}\NormalTok{(PrisonerIQ}\SpecialCharTok{$}\NormalTok{IQ) }

\CommentTok{\# Same with sd; it should be 15 but we calculate it to get a precise estimate and put in object \#‘sd’}
\NormalTok{sd}\OtherTok{\textless{}{-}}\FunctionTok{sd}\NormalTok{(PrisonerIQ}\SpecialCharTok{$}\NormalTok{IQ) }
...
\FunctionTok{pnormGC}\NormalTok{(}\DecValTok{124}\NormalTok{, }\AttributeTok{region=}\StringTok{"below"}\NormalTok{, }\AttributeTok{mean=}\NormalTok{m, }\AttributeTok{sd=}\NormalTok{sd,}\AttributeTok{graph=}\ConstantTok{TRUE}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
